defmodule SmithyClient do
  ...
  defp build_headers(%Operation{content_type: ct, static_headers: sh}, config) do
  ...
  defp encode_body(%Operation{
         encoding: :form,
         action: action,
         input: input,
         api_version: ver,
         rename_map: rename_map,
         nested_rename_map: nested_rename_map
       }) do
  ...
  # designated payload member as the raw HTTP body.
  ...
  defp send_request(op, url, headers, body) do
    method = op.http.method |> String.downcase() |> String.to_atom()

    case SmithyHttpClient.request(method, url, headers, body) do
  ...
  defp decode_response(:xml, body) when is_binary(body) do
    apply(SmithyXml, :decode, [body])
  end

  defp decode_response(:xml, body) when is_map(body), do: {:ok, body}

  # awsQuery / ec2Query XML decode — unwrap the {OperationResult -> ...} envelope.
  defp decode_response(:query_xml, body) when is_binary(body) and byte_size(body) == 0,
    do: {:ok, %{}}
  ...
  # Raw decode — @httpPayload operations return the body binary as-is.

...
- **`RestXmlProtocolAnalyzer`** in `codegen-protocols`: analyzes `aws.protocols#restXml` — same HTTP/label/query/header analysis as `RestJsonProtocolAnalyzer` but with `BodyEncoding.XML` and `ErrorCodeStrategy.REST_XML`. `requiresXmlRuntime()` returns `true`. `requiresS3Runtime(service)` returns `true` when the `aws.api#service` trait's `arnNamespace` starts with `"s3"`, triggering `aws_s3.erl` to be copied alongside `aws_xml.erl`.

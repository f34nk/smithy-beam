...
class ElixirWaiterIrTest {
  ...
  void waitBucketExistsFunctionMatchesExpectedShape() {
    BeamWaiterIndex.WaiterBinding binding =
        index.bindings().stream()
            ...
            .findFirst()
            .orElseThrow();
    Function fn = ElixirWaiterDsl.waiterFunction(index, binding, clientMod, typesMod, provider);
  ...
  void waitUntilHelpersIncludePollingLoop() {
    List<Function> helpers = ElixirWaiterDsl.waitUntilHelperFunctions();
    String combined =
  ...
  void waitersModuleMatchesGolden() throws IOException {
    Module module = ElixirWaiterDsl.waitersModule(testContext(), service, index, provider, model);
    assertThat(ElixirRenderer.render(module))
        .isEqualTo(readExpectedString("dsl/waiters_module.expected.ex"));
  }

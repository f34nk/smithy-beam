...
class ElixirDslGoldenIntegrationTest {
  ...
  @BeforeAll
  static void setup() {
    URL resource = ElixirDslGoldenIntegrationTest.class.getResource("/model/golden_http.smithy");
    ...
    assertThat(ElixirRenderer.render(nested))
        .isEqualTo(
            DslGoldenAssertions.readExpectedString(
                "dsl/golden/get_name_output_structure.expected.ex"));
  }

  @Test
  void clientCodecModuleFromSmithyMatchesGolden() throws IOException {
    var module = ElixirRestJsonDsl.buildClientCodecModule(clientContext(), service);
    DslGoldenAssertions.assertGolden(
        module, "dsl/golden/http_service_rest_json_1_client_codec.expected.ex");

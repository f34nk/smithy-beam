...
class ElixirFormatTest {
  ...
  void writeSpecSplitsLongSignatureAtDoubleColon() {
    ...
    assertThat(out)
        .contains("        {:ok, BasicServiceTypes.GetTypeClosureOutput.t()} | {:error, term()}");
  }
  ...
  void writeSpecKeepsShortSignatureOnOneLine() {
    String out =
        emit(
            w ->
                ElixirFormat.writeSpec(w, "@spec", "callbacks() :: [{atom(), non_neg_integer()}]"));
  ...
  void breakFunctionHeadSplitsLongDef() {
    ...
    assertThat(out)
        .contains("%RuntimeTypes.HttpRequest{query: query, headers: headers, body: body},");
  ...
  void writeStructureTypeAlignsClosingBrace() {
    String out =
        emit(
            w ->
                ElixirFormat.writeStructureType(
                    w,
                    List.of(
  ...
  void writeHeredocBodyUsesFourSpaceRelativeIndent() {
    ...
    assertThat(out)
        .contains("    Generated Elixir client for smithy.beam.demo.basic#BasicService.");
  ...
  void writeDefexceptionUsesKeywordLayout() {
    String out =
        emit(
            w ->
                ElixirFormat.writeDefexception(
        ...
        emit(
            w ->
                ElixirFormat.breakFunctionHead(
                    ...
                    List.of(
                        "%RuntimeTypes.HttpResponse{status: 200, headers: headers, body: body}")));

...
class ElixirS3EndpointDslTest {
  ...
  void regionHostMatchesExpectedShape() {
    Function fn = ElixirS3EndpointDsl.regionHost();
  ...
  void helperFunctionsMatchExpectedShape() {
    String combined =
        ElixirS3EndpointDsl.helperFunctions().stream()
            .map(ElixirRenderer::renderFunction)
            .collect(Collectors.joining("\n\n"));

    for (Function fn : ElixirS3EndpointDsl.helperFunctions()) {
      ElixirDslTestSupport.assertStructural(fn);
    }
  ...
  private static Model s3Model() {
    return Model.assembler()
        .addImport(ElixirS3EndpointDslTest.class.getResource("/model/s3_rest_xml_fixture.smithy"))
        .discoverModels()
        .assemble()
        .unwrap();
  }

...
class ElixirAwsJsonIrTest {
  ...
  void buildClientCodecModuleHasExpectedStructure() {
    Module module =
        ElixirAwsJsonDsl.buildClientCodecModule(
  ...
  void encodeGetUserRequestMatchesGolden() throws IOException {
    Function fn =
        ElixirAwsJsonOperationDsl.buildEncodeRequest(
                model,
                getUserOp,
                httpIndex,
                sp,
    ...
    assertThat(ElixirRenderer.renderFunction(fn))
        .isEqualTo(readExpectedString("dsl/aws_json_encode_get_user_request.expected.ex"));
  }

  @Test
  void decodeGetUserResponseMatchesGolden() throws IOException {
    Function fn =
        ElixirAwsJsonOperationDsl.buildDecodeResponse(
                model, getUserOp, httpIndex, sp, typesMod, runtimeMod, eventStreamModule)
            .get(0);
    ElixirDslTestSupport.assertStructural(fn);
    assertThat(ElixirRenderer.renderFunction(fn))
        .isEqualTo(readExpectedString("dsl/aws_json_decode_get_user_response.expected.ex"));
  }

  @Test
  void decodeGetUserRequestMatchesGolden() throws IOException {
    Function fn =
        ElixirAwsJsonOperationDsl.buildDecodeRequest(
                model, getUserOp, httpIndex, sp, typesMod, runtimeMod, eventStreamModule)
            .get(0);
    ElixirDslTestSupport.assertStructural(fn);
    assertThat(ElixirRenderer.renderFunction(fn))
        .isEqualTo(readExpectedString("dsl/aws_json_decode_get_user_request.expected.ex"));
  }

  @Test
  void encodeGetUserResponseMatchesGolden() throws IOException {
    Function fn =
        ElixirAwsJsonOperationDsl.buildEncodeResponse(
                model,
                getUserOp,
                httpIndex,
                sp,
    ...
    ElixirDslTestSupport.assertStructural(fn);
    assertThat(ElixirRenderer.renderFunction(fn))
        .isEqualTo(readExpectedString("dsl/aws_json_encode_get_user_response.expected.ex"));
  }

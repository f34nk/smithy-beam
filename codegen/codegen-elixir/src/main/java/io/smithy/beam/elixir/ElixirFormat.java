 ...
 *
 * <p>Encodes mix-format-style layout rules used across emitters so generated code matches idiomatic
 * Elixir without a post-processing pass.
 */
final class ElixirFormat {
  ...
  /**
   * Writes an {@code @spec} or {@code @callback} line, splitting at {@code ::} when the signature
   * exceeds {@link #SPEC_LINE_LIMIT} characters.
   */
  ...
  static void writeSpec(
      ElixirWriter writer, String annotation, String name, String params, String returnType) {
   ...
   */
  static void writeCallbackSpec(
      ElixirWriter writer, String name, List<String> params, String returnType) {
   ...
   *
   * <p>For multiline text, opens the heredoc, writes the body via {@link #writeHeredocBody}, then
   * closes the heredoc. Call when the writer is already at module-body indent.
   */
   ...
   * {@code @doc}. The heredoc opener must already be written; call before the closing {@code """}.
   */
   ...
   * Writes an optional list entry using an {@code if(...)} expression.
   *
   * <p>Uses a single-line form when {@code forceSingleLine} is true or the full expression fits
   * within {@link #IF_IN_LIST_SINGLE_LINE_LIMIT} characters.
   */
  static void writeIfInList(
      ElixirWriter writer, String condition, String doExpr, boolean forceSingleLine) {
  ...
  static void writePipeCaseField(
      ElixirWriter writer, String fieldName, String expr, List<String[]> cases) {
  ...
  /** Writes a standalone {@code expr |> case do ... end} block. */
  static void writePipeCase(
      ElixirWriter writer, String expr, List<String[]> cases, boolean blankBetweenBranches) {
    writer.write("$L", expr);
  ...
  static void breakFunctionHead(
      ElixirWriter writer, String keyword, String name, List<String> args) {
   ...
   *
   * <p>When an argument is a {@code %Struct{...}} pattern with multiple fields, the struct fields
   * are broken onto separate lines.
   */
  static void breakFunctionHead(
  ...
  /**
   * Writes a {@code defp} head with a multi-line {@code do:} body (for example expanded {@code
   * fn}).
   */

...
final class ElixirRestXmlDsl {
  ...
  static List<Function> xmlHelperFunctions(Optional<String> serviceNamespace) {
    ...
    functions.addAll(ElixirXmlCodecDsl.restXmlDecodeHelpers());
    return functions;
  }
  ...
  static List<Function> clientCodecFunctions(
    ...
    for (OperationShape op : operations) {
      functions.addAll(
          ElixirRestXmlOperationDsl.buildEncodeRequest(
              model, service, op, httpIndex, sp, typesMod, runtimeMod, encodeWithConfig));
      functions.addAll(
          ElixirRestXmlOperationDsl.buildDecodeResponse(model, op, httpIndex, sp, typesMod));
      functions.addAll(ElixirRestXmlOperationDsl.buildErrorDispatch(model, op, sp, typesMod));
    }
  ...
  static List<Function> serverCodecFunctions(
    ...
    for (OperationShape op : operations) {
      functions.addAll(
          ElixirRestXmlOperationDsl.buildDecodeRequest(model, op, httpIndex, sp, typesMod));
      functions.addAll(
          ElixirRestXmlOperationDsl.buildEncodeResponse(

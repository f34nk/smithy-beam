...
public final class ElixirAwsQueryEmitter {
  ...
  static void emitCodecModule(ElixirContext ctx, ServiceShape service, ShapeId protocolTraitId) {
    ...
    Set<StructureShape> inputShapes =
        operations.stream()
  ...
  static void emitServerCodecModule(
    ...
    Set<StructureShape> inputShapes =
        operations.stream()
            .map(op -> model.expectShape(op.getInputShape(), StructureShape.class))
            .collect(Collectors.toCollection(LinkedHashSet::new));
    Set<StructureShape> outputShapes =
        operations.stream()
            ...
            writer -> {
              ...
              for (OperationShape op : operations) {
                ...
                emitServerResponseEncoder(
                    writer, model, service, op, sp, typesMod, runtimeMod, ec2Query);
              }
  ...
  private static void emitServerResponseEncoder(
    ...
    ElixirFormat.writeSpec(
        writer, "@spec", "encode_" + opName + "_response", outputType, httpResponseType);
    ...
    if (ec2Query) {
      writer.write(
          "body = encode_xml(%{\"$L\" => result_content}, xml_namespace())", resultElement);
    } else {
      writer.write("body = wrap_aws_query_response(");
      writer.write(
          "  \"$L\", result_content, \"$L\", xml_namespace())", resultElement, responseElement);
    }
  ...
  private static void emitOutputToResultMap(
      ElixirWriter writer, Model model, SymbolProvider sp, String typesMod, StructureShape output) {
  ...
  private static void emitServerQueryDecodeHelpers(ElixirWriter writer, boolean ec2Query) {
    ...
    ElixirFormat.writePipelineStep(
        writer, "Enum.filter(fn {k, _} -> String.starts_with?(k, prefix) end)");
    ElixirFormat.writePipelineStep(
        writer,
        "Enum.sort_by(fn {k, _} -> k |> String.replace_prefix(prefix, \"\") |> String.to_integer() end)");
  ...
  private static void emitServerXmlEncodeHelpers(ElixirWriter writer, boolean ec2Query) {
    if (!ec2Query) {
      writer.write(
          "defp wrap_aws_query_response(result_name, result_content, response_name, xml_ns) do");
      writer.indent();
    ...
    ElixirFormat.writePipelineStep(writer, "Enum.reject(fn {_, v} -> is_nil(v) end)");
    ElixirFormat.writePipelineStep(
        writer, "Enum.map(fn {k, v} -> build_xml_element(k, v, %{}) end)}");
    writer.dedent();
    writer.dedent();
    writer.write("end");
    writer.write("");
  ...
  private static void emitDecoder(
    ...
    if (output.members().isEmpty()) {
      ...
      writer.indent();
      writer.write("{:ok, _result} -> {:ok, %Types.$L{}}", outputStruct);
      writer.write("{:error, {:missing_result, _}} -> {:ok, %Types.$L{}}", outputStruct);
    ...
    writer.write("");
    writer.write(
        "def decode_$L_response(%RuntimeTypes.HttpResponse{status: status, body: body}) do",
        opName);
    writer.indent();
  ...
  private static void emitOutputFields(
      ElixirWriter writer, Model model, SymbolProvider sp, StructureShape output) {
    ...
    for (int i = 0; i < members.size(); i++) {
      ...
      if (target instanceof ListShape listShape) {
        ...
        if (i < members.size() - 1) {
          writer.write(
              "  $L: xml_child_list(result, \"$L\", \"$L\"),", field, element, itemElement);
        } else {
  ...
  private static void emitFlattenInputClause(
      ElixirWriter writer, SymbolProvider sp, StructureShape input, boolean ec2Query) {
  ...
  private static void emitQueryHelpers(ElixirWriter writer, boolean ec2Query) {
    ...
    writer.write(
        "else: flatten_member(\"#{key}.entry.#{i}.key\", k) ++ flatten_member(\"#{key}.entry.#{i}.value\", v)");
    writer.dedent();
    writer.write("end)");
    writer.dedent();
    writer.write("end");
  ...
  private static void emitXmlHelpers(ElixirWriter writer, boolean ec2Query) {
    ...
    writer.write("");
    writer.write("defp query_result_element(element, result_name) do");
    writer.indent();
    writer.write("if is_element(element) and element_name(element) == result_name do");
    writer.indent();
    writer.write("element");
    writer.dedent();
    writer.write("else");
    writer.indent();
    writer.write("find_element(result_name, element_content(element))");
    writer.dedent();
    writer.write("end");
    writer.dedent();
    writer.write("end");
    ...
    writer.indent();
    writer.write(
        "Enum.filter(content, fn item -> is_list(item) and not is_element_string(item) end)");
    writer.dedent();
    writer.write("end");
  ...
  private static void emitAwsQueryErrorDecoder(ElixirWriter writer) {
    ...
    writer.dedent();
    writer.write(
        "case query_result_element(xml, \"$L\") do", BeamXmlDecoder.ERROR_RESPONSE_ELEMENT);
    writer.indent();
    writer.write("nil ->");
    writer.indent();
    ...
    writer.indent();
    writer.write(
        "case find_element(\"$L\", element_content(error_response)) do",
  ...
  private static void emitEc2QueryErrorDecoder(ElixirWriter writer) {
    ...
    writer.write("end");
    writer.dedent();
    writer.write("case query_result_element(xml, \"$L\") do", BeamXmlDecoder.EC2_RESPONSE_ELEMENT);
    writer.indent();
    writer.write("nil -> {:error, {:unknown_error, status, body}}");
    writer.write("response ->");
    writer.indent();
    writer.write(
        "case find_element(\"$L\", element_content(response)) do",
    ...
    writer.indent();
    writer.write(
        "case find_element(\"$L\", element_content(errors)) do", BeamXmlDecoder.ERROR_ELEMENT);
    writer.indent();
    writer.write("nil -> {:error, {:unknown_error, status, body}}");
    writer.write("error -> {:error, {");
    writer.write("  xml_child_text(error, \"$L\"),", BeamXmlDecoder.ERROR_CODE_ELEMENT);

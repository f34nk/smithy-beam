...
/**
 * Emits {@code <service>_credentials.ex} with a default AWS credential resolution chain
...
public final class ElixirCredentialProviderEmitter {
    ...
    private static void writeResolveChain(ElixirWriter writer) {
        ...
        for (BeamCredentialProviderKind kind : BeamCredentialProviders.defaultChain()) {
            writer.write("defp resolve_provider(:$L, config), do: resolve_from_$L(config)",
                    elixirProviderAtom(kind), elixirProviderSuffix(kind));
        }
        writer.write("");
    }
    ...
    private static void writeResolveFromEnv(ElixirWriter writer) {
        ...
        writer.indent();
        writer.write("case {System.get_env(\"AWS_ACCESS_KEY_ID\"), System.get_env(\"AWS_SECRET_ACCESS_KEY\")} do");
        writer.indent();
    ...
    private static void writeResolveFromEc2(ElixirWriter writer) {
        ...
        writer.indent();
        writer.write("with {:ok, role_bin} <- ec2_metadata_request(\"/latest/meta-data/iam/security-credentials/\"),");
    ...
    private static void writeSharedHelpers(ElixirWriter writer) {
        ...
        writer.write("end");
        writer.write("");
        writer.write("defp map_to_credentials(%{\"aws_access_key_id\" => id, \"aws_secret_access_key\" => secret} = fields) do");
        writer.indent();
        writer.write("{:ok, %{");

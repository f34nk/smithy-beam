...
public final class ElixirComplianceTestEmitter {
    ...
    public static void emit(ElixirContext ctx, ServiceShape service) {
        ...
        boolean encodeWithConfig = ElixirRestJson1Emitter.serviceHasHostLabelOperations(model, service)
    ...
    private static void emitClientRequestTest(
        ...
        String inputLiteral = BeamComplianceLiterals.elixirStructLiteral(
        ...
        if (!testCase.queryParams().isEmpty()) {
            writer.write("assert_query_params($L, request.query)",
                    BeamComplianceLiterals.elixirQueryParamsList(testCase.queryParams()));
        }
        if (!testCase.headers().isEmpty()) {
            writer.write("assert_headers($L, request.headers)",
                    BeamComplianceLiterals.elixirHeadersMap(testCase.headers()));
        }
        if (testCase.body() != null) {
            writer.write("assert IO.iodata_to_binary(request.body) == \"$L\"", escapeElixir(testCase.body()));
        }
        writer.dedent();
        writer.write("end");
        writer.write("");
    ...
    private static void emitServerRequestTest(
        ...
        writer.write("query: query_params_to_map($L),", BeamComplianceLiterals.elixirQueryParamsList(testCase.queryParams()));
        writer.write("headers: headers_to_list($L),", BeamComplianceLiterals.elixirHeadersMap(testCase.headers()));
    ...
    private static void emitServerResponseTest(
            ...
            boolean errorCase,
            Function<StructureShape, String> structNameFn) {
        String outputLiteral = BeamComplianceLiterals.elixirStructLiteral(
        ...
        if (!testCase.headers().isEmpty()) {
            writer.write("assert_headers($L, response.headers)",
                    BeamComplianceLiterals.elixirHeadersMap(testCase.headers()));
        }
        if (testCase.body() != null) {
            writer.write("assert IO.iodata_to_binary(response.body) == \"$L\"", escapeElixir(testCase.body()));
        }
        writer.dedent();
        writer.write("end");
        writer.write("");
    ...
    private static void assertParamsOnStruct(
        ...
        for (var entry : params.getMembers().entrySet()) {
            ...
            shape.getMember(memberName).ifPresent(member -> {
                ...
                String expected = BeamComplianceLiterals.elixirMemberValue(

...
class BeamWaiterPathsTest {
  ...
  void simpleDottedPathEmitsSnakeCaseSegments() {
    ...
    assertThat(BeamWaiterPaths.emitErlangPath("Table.TableStatus"))
        .isEqualTo("[table, table_status]");
    assertThat(BeamWaiterPaths.emitElixirPath("Table.TableStatus"))
        .isEqualTo("[:table, :table_status]");
  }
  ...
  void complexPathEmitsStringLiteral() {
    ...
    assertThat(BeamWaiterPaths.emitErlangPath(path))
        .isEqualTo("<<\"" + path.replace("\\", "\\\\").replace("\"", "\\\"") + "\">>");

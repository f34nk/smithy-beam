...
class DeprecatedShapeFilterTest {
    ...
    void relativeDateRemovesDeprecatedShapesFromClientGeneration() {
        ...
        executeClientPlugin(model, baseline, ObjectNode.builder()
                .withMember("service", SERVICE)
                .withMember("edition", "2026")
                .build());
        assertThat(baseline.expectFileString(TYPES_FILE)).contains("-type legacy_string() :: binary().");

        MockManifest filtered = new MockManifest();
        executeClientPlugin(model, filtered, ObjectNode.builder()
                .withMember("service", SERVICE)
                .withMember("edition", "2026")
                .withMember("relativeDate", "2026-01-01")
                .build());
        assertThat(filtered.expectFileString(TYPES_FILE)).doesNotContain("-type legacy_string() :: binary().");
    }

...
class EmptyEnumTypesTest {
  ...
  void erlangTypesEmitValidAliasesForEmptyEnumMembers() throws Exception {
    ...
    SymbolProvider emptyAtomProvider =
        shape -> {
    ...
    Method generateEnum =
        codegen.getClass().getDeclaredMethod("generateEnumShape", GenerateEnumDirective.class);
    generateEnum.setAccessible(true);
    Method generateIntEnum =
        codegen
            .getClass()
    ...
    generateEnum.invoke(
        codegen, newEnumDirective(context, service, model.expectShape(STATUS_ID, EnumShape.class)));
  ...
  private static Model modelWithPopulatedEnums() {
    ServiceShape service =
        ServiceShape.builder().id(SERVICE_ID).version("1").addOperation(OP_ID).build();
    OperationShape operation = OperationShape.builder().id(OP_ID).output(BUNDLE_ID).build();
    EnumShape status = EnumShape.builder().id(STATUS_ID).addMember("ACTIVE", "ACTIVE").build();
    IntEnumShape priority = IntEnumShape.builder().id(PRIORITY_ID).addMember("LOW", 1).build();

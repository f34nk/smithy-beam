...
final class ErlangRestXmlOperationIr {
    ...
    return ErlFunction.functionWithDocAndSpec(
        ...
        List.of(
            ErlClause.clause(
                patterns,
                ErlExprBlock.block(
                    buildEncodeRequestBodyExprs(model, service, op, httpIndex, sp, encodeWithConfig)
                        .toArray(ErlExpr[]::new)))));
  }
    ...
    return ErlFunction.functionWithDocAndSpec(
        ...
        List.of(
            ErlClause.clause(
                patterns,
                ErlExprBlock.block(
                    buildDecodeRequestBodyExprs(model, op, httpIndex, sp)
                        .toArray(ErlExpr[]::new)))));
  }
  ...
  static List<ErlFunction> buildDecodeResponse(
    ...
    List<ErlPattern> successPatterns =
        List.of(
    ...
    List<ErlPattern> fallbackPatterns =
        List.of(
            ErlRecordPattern.recordPattern(
                "http_response",
    ...
    clauses.add(
        ErlClause.clause(
            successPatterns,
            ErlExprBlock.block(
                buildDecodeResponseSuccessBodyExprs(model, op, httpIndex, sp)
                    .toArray(ErlExpr[]::new))));
    clauses.add(ErlClause.clause(fallbackPatterns, buildDecodeResponseFallbackExprs(op, sp)));

    List<ErlFunction> functions = new ArrayList<>();
    functions.add(
        ErlFunction.functionWithDocAndSpec(
            "decode_" + opName + "_response",
            1,
  ...
  static ErlFunction buildErrorResponseEncoder(Model model, ShapeId errorId, SymbolProvider sp) {
    ...
    ErlExpr body =
        ErlExprBlock.block(
            ErlMatch.match(
                ErlVarPattern.varPattern("XmlNs"), ErlCallLocal.callLocal("xml_namespace")),
            ErlMatch.match(
    ...
    return ErlFunction.function(
        ...
        1,
        List.of(ErlClause.clause(List.of(new ErlRecordPattern(recName, List.of())), body)));
  }
    ...
    if (!respPayload.isEmpty()) {
      ...
      body.add(
          ErlMatch.match(
              ...
              ErlCase.caseExpr(
                  ErlCallLocal.callLocal(
                      "parse_xml_root", ErlVar.var("Body"), ErlBinary.binary(rootElement)),
                  ErlClause.clause(
                      List.of(
                          ErlTuplePattern.tuplePattern(
                              ErlAtomPattern.atomPattern("ok"), ErlVarPattern.varPattern("Root"))),
                      ErlVar.var("Root")),
                  ErlClause.clause(
                      List.of(
                          ErlTuplePattern.tuplePattern(
    ...
    if (s3BucketAddressing) {
      ...
      if (encodeWithConfig) {
        body.add(
            ErlMatch.match(
                ErlTuplePattern.tuplePattern(
                    ErlVarPattern.varPattern("Host"), ErlVarPattern.varPattern("Path")),
                ErlCall.call(
                    "s3_endpoint",
                    "resolve_bucket_url",
                    ErlVar.var("Config"),
      ...
      body.add(
          ErlMatch.match(
              ErlVarPattern.varPattern("Path"), buildPathExpression(uriTemplate, labels)));
    }
  ...
  static List<ErlExpr> buildEncodeResponseBodyExprs(
    ...
    if (respHeaders.isEmpty()) {
      body.add(
          ErlMatch.match(
              ...
              ErlList.list(
                  ErlTuple.tuple(
                      ErlBinary.binary("Content-Type"), ErlBinary.binary("application/xml")))));
    } else {
      body.add(
          ErlMatch.match(
    ...
    if (!respPayload.isEmpty()) {
      ...
      for (MemberShape member : output.members()) {
        ...
        entries.add(
            ErlMapEntry.entry(
                ErlBinary.binary(wireName), ErlVar.var(ErlangRestXmlSupport.toBindingVar(field))));
      }
      body.add(
          ErlMatch.match(
              ErlVarPattern.varPattern("MemberMap"),
  ...
  private static ErlMatch headerBindingDecodeExpr(
    ...
    return List.of(
        ErlMatch.match(
            ...
            ErlCase.caseExpr(
                ErlVar.var("Body"),
                ErlClause.clause(
                    List.of(ErlBinaryPattern.binaryPattern("")), ErlAtom.atom("undefined")),
    ...
    if (target instanceof StructureShape structure) {
      ...
      payloadValueExpr =
          ErlCallLocal.callLocal(
              "encode_xml",
              ErlMap.map(
                  ErlMapEntry.entry(ErlBinary.binary(rootElement), ErlVar.var("PayloadValue"))),
              ErlVar.var("XmlNs"));
    }
        ...
        ErlMatch.match(
            ...
            ErlCase.caseExpr(
                ...
                ErlClause.clause(
                    List.of(ErlAtomPattern.atomPattern("undefined")), ErlBinary.binary("")),
                ErlClause.clause(
                    List.of(ErlVarPattern.varPattern("PayloadValue")), payloadValueExpr))));
    return exprs;
  }
          ...
          ErlClause.clause(
              ...
              ErlCallLocal.callLocal(
                  ...
                  ErlMap.map(
                      ErlMapEntry.entry(
                          ErlBinary.binary(rootElement),
                          ErlMap.map(ErlMapEntry.entry(ErlBinary.binary(element), innerValue)))),
                  ErlVar.var("XmlNs"))));
    }
    clauses.add(
      ...
      cases.add(
          ErlCase.caseExpr(
              ...
              ErlClause.clause(
                  ...
                  ErlList.list(
                      ErlTuple.tuple(ErlBinary.binary(binding.getLocationName()), encodedValue)))));
    }
      ...
      segments.add(
          ErlBinaryExpr.expr(
              ErlCallLocal.callLocal(
                  "uri_encode",
                  ErlCallLocal.callLocal(
                      "to_binary", ErlVar.var(ErlangRestXmlSupport.toBindingVar(fieldName)))),
              true));
      pos = end + 1;
    }

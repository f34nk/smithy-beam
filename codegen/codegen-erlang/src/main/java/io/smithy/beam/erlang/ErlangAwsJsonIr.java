...
final class ErlangAwsJsonIr {
    private static final Map<ShapeId, String> CONTENT_TYPES = Map.of(
            BeamProtocolIds.AWS_JSON_1_0, "application/x-amz-json-1.0",
            BeamProtocolIds.AWS_JSON_1_1, "application/x-amz-json-1.1");

    private static final Map<ShapeId, String> VERSION_LABELS = Map.of(
    ...
    static ErlModule buildClientCodecModule(ErlangContext ctx, ServiceShape service, ShapeId protocol) {
    ...
    static ErlModule buildServerCodecModule(ErlangContext ctx, ServiceShape service, ShapeId protocol) {
        Model model = ctx.model();
        BeamErlangLayout layout = layout(ctx, service);
        HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
        SymbolProvider sp = ctx.symbolProvider();

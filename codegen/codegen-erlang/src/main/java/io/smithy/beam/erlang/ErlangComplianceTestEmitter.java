...
public final class ErlangComplianceTestEmitter {
    ...
    public static void emit(ErlangContext ctx, ServiceShape service) {
        ...
        ctx.writerDelegator().useFileWriter(layout.complianceTestsModuleFile(), writer -> {
            writer.write("%% HTTP protocol compliance tests for $L (generated).", service.getId());
            ...
            for (BeamHttpComplianceTests.OperationRequestTests binding : requestBindings) {
                ...
                List<HttpBinding> labels = httpIndex.getRequestBindings(operation, HttpBinding.Location.LABEL);
            ...
            for (BeamHttpComplianceTests.OperationResponseTests binding : responseBindings) {
                OperationShape operation = binding.operation();
                Symbol opSym = sp.toSymbol(operation);
                StructureShape outputShape = binding.errorShape()
    ...
    private static void emitClientRequestTest(
        ...
        String inputLiteral = BeamComplianceLiterals.erlangRecordLiteral(model, input, testCase.params(), sp);
        String encodeCall = encodeWithConfig
                ? codecMod + ":encode_" + opSym.getName() + "_request(#{region => <<\"us-east-1\">>}, " + inputLiteral + ")"
                : codecMod + ":encode_" + opSym.getName() + "_request(" + inputLiteral + ")";

        writer.write("$L() ->", fn);
        writer.indent();
        writer.write("Request = $L,", encodeCall);
        writer.write("?assertEqual(<<\"$L\">>, Request#http_request.method),", escapeErlang(testCase.method()));
        writer.write("?assertEqual(<<\"$L\">>, Request#http_request.path),", escapeErlang(testCase.uri()));
        if (!testCase.queryParams().isEmpty()) {
            writer.write("assert_query_params($L, Request#http_request.query),",
                    BeamComplianceLiterals.erlangQueryParamsList(testCase.queryParams()));
        }
        if (!testCase.headers().isEmpty()) {
            writer.write("assert_headers($L, Request#http_request.headers),",
                    BeamComplianceLiterals.erlangHeadersMap(testCase.headers()));
        }
        if (testCase.body() != null) {
            writer.write("?assertEqual(<<\"$L\">>, iolist_to_binary(Request#http_request.body)),",
    ...
    private static void emitServerRequestTest(
        ...
        writer.write("query = query_params_to_map($L),", BeamComplianceLiterals.erlangQueryParamsList(testCase.queryParams()));
        writer.write("headers = headers_to_proplist($L),", BeamComplianceLiterals.erlangHeadersMap(testCase.headers()));
    ...
    private static void emitServerResponseTest(
        ...
        String outputLiteral = BeamComplianceLiterals.erlangRecordLiteral(model, outputShape, testCase.params(), sp);
        String encodeFn = errorCase
        ...
        if (!testCase.headers().isEmpty()) {
            writer.write("assert_headers($L, Response#http_response.headers),",
                    BeamComplianceLiterals.erlangHeadersMap(testCase.headers()));
        }
        if (testCase.body() != null) {
            writer.write("?assertEqual(<<\"$L\">>, iolist_to_binary(Response#http_response.body)),",
                    escapeErlang(testCase.body()));
        }
        writer.write("ok.");
        writer.dedent();
    ...
    private static void assertParamsOnRecord(
        ...
        for (var entry : params.getMembers().entrySet()) {
            ...
            shape.getMember(memberName).ifPresent(member -> {
                String fieldName = BeamNameUtils.toSnakeCase(memberName);
                String expected = BeamComplianceLiterals.erlangMemberValue(
                        model, member, entry.getValue(), sp);

...
final class ErlangRouterIr {
    ...
    static ErlModule routerModule(
        ...
        if (BeamProtocolIds.AWS_JSON_1_0.equals(protocol)
            ...
            preamble = List.of(
                    ErlComment.comment("Generated AWS JSON 1.0 router for " + service.getId() + "."));
            functions = List.of(
                    awsJsonDispatch(serverMod),
                    awsJsonRoute(service, operations, sp, codecMod));
        } else {
            preamble = List.of(ErlComment.comment("Generated HTTP router for " + service.getId() + "."));
            HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
            functions = List.of(
                    httpDispatch(serverMod),
                    httpRoute(httpIndex, operations, sp, codecMod, helpersMod));
        }
    ...
    private static ErlFunction httpDispatch(String serverMod) {
        ErlFunctionDoc doc = ErlFunctionDoc.functionDoc(
    ...
    private static ErlFunction awsJsonRoute(
        ...
        for (OperationShape op : operations) {
            ...
            postClauses.add(ErlClause.blockClause(
        ...
        postClauses.add(ErlClause.clause(
                List.of(ErlVarPattern.varPattern("_")),
                ErlTuple.tuple(
                        ErlAtom.atom("error"),
                        ErlTuple.tuple(
                                ErlAtom.atom("not_found"),
                                ErlBinary.binary("POST"),
                                ErlBinary.binary("/")))));

        ErlCase targetCase = ErlCase.caseExpr(
                ErlCall.call(
                        "proplists",
                        "get_value",
    ...
    private static ErlCase labeledRouteBody(
        ...
        return ErlCase.caseExpr(
                ...
                ErlClause.blockClause(
                        List.of(ErlTuplePattern.tuplePattern(
                                ErlAtomPattern.atomPattern("error"), ErlAtomPattern.atomPattern("path_mismatch"))),
                        ErlTuple.tuple(
                                ErlAtom.atom("error"),
                                ErlTuple.tuple(
                                        ErlAtom.atom("not_found"),
                                        ErlBinary.binary(method),
                                        ErlVar.var("Path")))));
    }
    ...
    private static List<ErlGuard> trailingLabelGuard(String uriTemplate) {
        ...
        return List.of(ErlGuard.exprGuard(

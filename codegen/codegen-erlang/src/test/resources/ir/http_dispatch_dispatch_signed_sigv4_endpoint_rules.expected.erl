dispatch_signed(HttpClient, Config, #http_request{method = Method, path = Path, query = Query, headers = Headers, body = Body, host = Host}) ->
    Config1 = case maps:get(credentials, Config, undefined) of
        undefined ->
    ...
    BaseUrl = case maps:get(base_url, Config1, undefined) of
        undefined ->
            case maps:get(endpoint_prefix, Config1, undefined) of
                undefined -> <<>>;
                _ ->
                    case endpoints:resolve(Config1, #{}) of
    ...
    QueryStr = case maps:to_list(Query) of
        [] -> <<>>;
        Pairs ->
    ...
    case HttpClient:request(binary_to_atom(string:lowercase(Method), utf8), Req, [], [{body_format, binary}]) of

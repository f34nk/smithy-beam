...
element_name({xmlElement, Name, _, _, _, _, _, _, _, _, _, _}) when is_atom(Name) -> list_to_binary(atom_to_list(Name));
element_name({xmlElement, Name, _, _, _, _, _, _, _, _, _, _}) when is_list(Name) -> list_to_binary(Name);
...
element_text({xmlElement, _, _, _, _, _, _, _, Content, _, _, _}) -> xml_text_values(Content);
...
xml_child_list(Parent, undefined, ItemName) -> [ItemText || Item <- element_content(Parent), is_element(Item), element_name(Item) =:= ItemName, ItemText <- [list_to_binary(element_text(Item))], ItemText =/= <<>>];
...
decode_rest_xml_error(Status, Body) ->
    case parse_xml_root(Body, <<"ErrorResponse">>) of
        ...
        {error, _} -> {error, {unknown_error, Status, Body}}
    end.
...
build_xml_element(Name, Content, XmlNs) when is_map(Content) ->
    ...
    {Name, Attrs, Children};
build_xml_element(Name, Content, XmlNs) -> {Name, xml_namespace_attrs(XmlNs), [{text, to_binary(Content)}]}.

build_xml_child(Name, Value) when is_map(Value) -> {Name, [], [build_xml_element(K, V, #{}) || {K, V} <- maps:to_list(Value), V =/= undefined]};
build_xml_child(Name, Values) when is_list(Values) -> {Name, [], [build_xml_element(<<"member">>, V, #{}) || V <- Values, V =/= undefined]};
build_xml_child(Name, Value) -> {Name, [], [{text, to_binary(Value)}]}.

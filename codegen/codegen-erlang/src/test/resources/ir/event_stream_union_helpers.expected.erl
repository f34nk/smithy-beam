...
encode_event_stream_event({member, Value}) ->
    Payload =
        jsone:encode(maps:filter(fun(_, V) -> V =/= undefined end, #{
                <<"value">> => Value#member_event.value
            })),
    Headers = encode_event_headers(<<"member">>),
    aws_event_stream:frame(Headers, Payload);
encode_event_stream_event({unknown, _}) -> error({bad_event, unknown}).
...
decode_event_stream_event_type(<<"member">>, Payload) -> {member, begin
        Decoded = jsone:decode(Payload, [return_maps]),
        #member_event{
            value = maps:get(<<"value">>, Decoded, undefined)
        }
    end};

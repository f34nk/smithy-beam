...
class ErlangDslGoldenIntegrationTest {
  ...
  @BeforeAll
  static void setup() {
    URL resource = ErlangDslGoldenIntegrationTest.class.getResource("/model/golden_http.smithy");
  ...
  void structureTypeHeaderFromSmithyMatchesGolden() throws IOException {
    ...
    DslGoldenAssertions.assertGolden(header, "dsl/get_name_output_structure.expected.hrl");
  }

  @Test
  void clientCodecModuleFromSmithyMatchesGolden() throws IOException {
    Module module = ErlangRestJsonDsl.buildClientCodecModule(clientContext(), service);
    DslGoldenAssertions.assertGolden(
        module, "dsl/http_service_rest_json_1_client_codec.expected.erl");

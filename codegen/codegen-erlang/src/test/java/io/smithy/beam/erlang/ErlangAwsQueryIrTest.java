...
class ErlangAwsQueryIrTest {
    ...
    static void setup() {
        ...
        service = model.expectShape(ShapeId.from("smithy.beam.test.awsquery#QueryService"), ServiceShape.class);
        listUsersOp = model.expectShape(ShapeId.from("smithy.beam.test.awsquery#ListUsers"), OperationShape.class);
    ...
    void encodeListUsersRequestMatchesGolden() throws IOException {
        ErlFunction fn = ErlangAwsQueryIr.encodeRequest(model, service, listUsersOp, httpIndex, provider);
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_encode_list_users_request.expected.erl"));
    }
    ...
    void decodeListUsersResponseMatchesGolden() throws IOException {
        ...
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_decode_list_users_response.expected.erl"));
    }

    @Test
    void flattenQueryInputMatchesGolden() throws IOException {
        Model flattenModel = flattenModel();
        ServiceShape flattenService = flattenModel.expectShape(
        ...
        BeamErlangLayout layout = new BeamErlangLayout(
                settings, flattenService.getId().getNamespace(), flattenService);
        ErlangSymbolProvider flattenProvider = new ErlangSymbolProvider(
                settings, flattenModel, flattenService, layout.clientModuleFile(), BeamCodegenKind.CLIENT);
        ErlFunction fn = ErlangAwsQueryIr.flattenQueryInput(
                flattenModel,
                ...
                false);
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_flatten_query_input.expected.erl"));
    }
    ...
    void serverDecodeListUsersRequestMatchesGolden() throws IOException {
        ...
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_server_decode_list_users_request.expected.erl"));
    }

    @Test
    void serverEncodeListUsersResponseMatchesGolden() throws IOException {
        ErlFunction fn = ErlangAwsQueryIr.serverEncodeResponse(model, service, listUsersOp, provider, false);
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_server_encode_list_users_response.expected.erl"));
    }
    ...
    void parseListUsersInputMatchesGolden() throws IOException {
        ...
        assertStructural(fn);
        assertThat(fn.asString()).isEqualTo(readExpectedString("ir/aws_query_parse_list_users_input.expected.erl"));
    }

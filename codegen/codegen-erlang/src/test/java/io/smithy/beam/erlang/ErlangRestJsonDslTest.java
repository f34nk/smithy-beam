...
class ErlangRestJsonIrTest {
  ...
  void structureDecodeEncodeAsStringMatchesGolden() throws IOException {
    ...
    List<Function> functions =
        ErlangRestJsonDsl.buildStructureDecodeEncode(model, httpIndex, basicItem, provider);
    ...
    assertThat(combined)
        .isEqualTo(
            DslGoldenAssertions.readExpectedString(
                "dsl/structure_decode_encode_basic_item.expected.erl"));
  }
  ...
  void encodeGetNameRequestMatchesGolden() throws IOException {
    ...
    HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
    Function fn =
        ErlangRestJsonDsl.encodeRequest(
            model, service, op, httpIndex, sp, false, layout.eventStreamModuleName());
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/rest_json_encode_get_name_request.expected.erl");
  }
  ...
  void decodedBodyPreludeMatchesGolden() throws IOException {
    ...
    assertThat(combined)
        .isEqualTo(
            DslGoldenAssertions.readExpectedString("dsl/json_decoded_body_prelude.expected.erl"));
  }
  ...
  void bodyMapEntriesForBasicItemMatchesGolden() throws IOException {
    ...
    assertThat(combined)
        .isEqualTo(
            DslGoldenAssertions.readExpectedString(
                "dsl/json_body_map_entries_basic_item.expected.erl"));
  }
  ...
  void encodeResponseBodyExprsMatchGolden() throws IOException {
    ...
    HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
    String combined =
        ErlangRestJsonOperationDsl.buildEncodeResponseBodyExprs(model, op, httpIndex, sp).stream()
            .map(ErlangRenderer::renderExpression)
            .collect(Collectors.joining("\n"));
    assertThat(combined)
        .isEqualTo(
            DslGoldenAssertions.readExpectedString(
                "dsl/rest_json_encode_response_body.expected.erl"));
  }
  ...
  void encodeGetNameResponseMatchesGolden() throws IOException {
    ...
    ErlangSymbolProvider sp =
        new ErlangSymbolProvider(
            settings, model, service, "runtime_types.hrl", BeamCodegenKind.CLIENT);
    HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
    Function fn = ErlangRestJsonOperationDsl.buildEncodeResponse(model, op, httpIndex, sp);
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/rest_json_encode_get_name_response.expected.erl");
  }
  ...
  void encodeNotFoundErrorResponseMatchesGolden() throws IOException {
    ...
    ErlangSymbolProvider sp =
        new ErlangSymbolProvider(
            settings, model, service, "runtime_types.hrl", BeamCodegenKind.CLIENT);
    Function fn = ErlangRestJsonOperationDsl.buildErrorResponseEncoder(model, errorId, sp);
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(
        fn, "dsl/rest_json_encode_not_found_error_response.expected.erl");
  }
  ...
  void decodeGetNameResponseMatchesGolden() throws IOException {
    ...
    ErlangSymbolProvider sp =
        new ErlangSymbolProvider(
            settings, model, service, layout.clientModuleFile(), BeamCodegenKind.CLIENT);
    HttpBindingIndex httpIndex = HttpBindingIndex.of(model);
    Function fn = ErlangRestJsonDsl.decodeResponse(model, service, op, httpIndex, sp, layout);
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/rest_json_decode_get_name_response.expected.erl");
  }
  ...
  void structureListDecodeEncodeAsStringMatchesGolden() throws IOException {
    List<Function> functions =
        ErlangRestJsonDsl.buildStructureListDecodeEncodeFunctions(basicItem, provider);
    assertThat(functions).hasSize(2);
    assertStructural(functions.get(0));
    assertStructural(functions.get(1));
    String combined =
    ...
    assertThat(combined)
        .isEqualTo(
            DslGoldenAssertions.readExpectedString(
                "dsl/structure_list_decode_encode_item.expected.erl"));
  }

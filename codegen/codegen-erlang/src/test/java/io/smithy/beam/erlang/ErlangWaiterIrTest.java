...
class ErlangWaiterIrTest {
    ...
    void waitBucketExistsFunctionMatchesExpectedShape() {
        BeamWaiterIndex.WaiterBinding binding = index.bindings().stream()
        ...
        assertThat(text).contains("Waits using the BucketExists waiter on smithy.beam.test.waiters#HeadBucket.");
    ...
    void waitUntilHelpersIncludePollingLoop() {
        ...
        String combined = helpers.stream().map(ErlFunction::asString).collect(Collectors.joining("\n\n"));
    ...
    void waitersModuleIncludesExportsAndWaiters() {
        ErlModule module = ErlangWaiterIr.waitersModule(

...
class ErlangAwsJsonIrTest {
  ...
  void buildClientCodecModuleHasExpectedStructure() {
    Module module =
        ErlangAwsJsonDsl.buildClientCodecModule(
  ...
  void encodeGetUserRequestMatchesGolden() throws IOException {
    Function fn =
        ErlangAwsJsonDsl.encodeRequest(
            model,
            getUserOp,
            httpIndex,
            provider,
            ...
            "event_stream");
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/aws_json_encode_get_user_request.expected.erl");
  }

  @Test
  void decodeGetUserResponseMatchesGolden() throws IOException {
    Function fn =
        ErlangAwsJsonDsl.decodeResponse(model, getUserOp, httpIndex, provider, "event_stream");
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/aws_json_decode_get_user_response.expected.erl");
  }

  @Test
  void errorDispatchGetUserMatchesGolden() throws IOException {
    Function fn = ErlangAwsJsonDsl.errorDispatch(model, getUserOp, provider);
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/aws_json_error_dispatch_get_user.expected.erl");
  }

  @Test
  void decodeGetUserRequestMatchesGolden() throws IOException {
    Function fn =
        ErlangAwsJsonDsl.decodeRequest(model, getUserOp, httpIndex, provider, "event_stream");
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/aws_json_decode_get_user_request.expected.erl");
  }

  @Test
  void encodeGetUserResponseMatchesGolden() throws IOException {
    Function fn =
        ErlangAwsJsonDsl.encodeResponse(
            model, getUserOp, httpIndex, provider, "application/x-amz-json-1.1", "event_stream");
    assertStructural(fn);
    DslGoldenAssertions.assertGolden(fn, "dsl/aws_json_encode_get_user_response.expected.erl");
  }

  @Test
  void sharedCodecHelpersMatchesGolden() throws IOException {
    List<Function> functions = ErlangAwsJsonDsl.sharedCodecHelpers(model, service, provider);
    DslGoldenAssertions.assertGoldenFunctions(
        functions, "dsl/aws_json_shared_codec_helpers.expected.erl");

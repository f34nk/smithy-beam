...
class ErlangWaiterIrTest {
  ...
  void waitBucketExistsFunctionMatchesExpectedShape() {
    BeamWaiterIndex.WaiterBinding binding =
        index.bindings().stream()
            ...
            .findFirst()
            .orElseThrow();
    Function fn = ErlangWaiterDsl.waiterFunction(index, binding, clientMod, provider);
  ...
  void waitUntilHelpersIncludePollingLoop() {
    List<Function> helpers = ErlangWaiterDsl.waitUntilHelperFunctions(model, service, provider);
    String combined = DslGoldenAssertions.renderFunctions(helpers);

...
class ErlangEndpointRulesIrTest {
    ...
    void mergeParamsFunctionsMatchGolden() throws IOException {
        ...
        String combined = ErlangEndpointRulesIr.mergeParamsFunctions(clientContextKeys).stream()
                .map(ErlFunction::asString)
                .collect(Collectors.joining("\n\n"));
        assertThat(combined).isEqualTo(readExpectedString("ir/endpoint_rules_merge_params.expected.erl"));
    }
    ...
    void endpointRulesModuleMatchesGolden() throws IOException {
        ServiceShape service = endpointModel().expectShape(ENDPOINT_SERVICE, ServiceShape.class);
        var clientContextKeys = BeamContextParamsIndex.clientContextConfigKeys(service);
        ErlModule module = ErlangEndpointRulesIr.endpointRulesModule(
                "endpoint_rules_service_endpoints",
                "runtime_types.hrl",
                service,
                clientContextKeys);
    ...
    private static Model endpointModel() {
        return Model.assembler()
                .addImport(ErlangEndpointRulesIrTest.class.getResource("/model/endpoint_rules_minimal.smithy"))
                .discoverModels()
                .assemble()
                .unwrap();
    }
    ...
    private static String readExpectedString(String resourcePath) throws IOException {
        try (InputStream in = ErlangEndpointRulesIrTest.class.getClassLoader().getResourceAsStream(resourcePath)) {

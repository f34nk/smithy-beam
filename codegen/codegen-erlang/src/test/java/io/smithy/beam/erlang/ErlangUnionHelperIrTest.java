...
class ErlangUnionHelperIrTest {
    ...
    static void setup() {
        ...
        Model model = Model.assembler()
                .addUnparsedModel("event.smithy", idl)
                .assemble()
                .unwrap();
        ServiceShape service = model.expectShape(ShapeId.from("com.example#EventService"), ServiceShape.class);
        BeamSettings settings = new BeamSettings();
        settings.edition("2026");
        provider = new ErlangSymbolProvider(settings, model, service, "event_types.hrl", BeamCodegenKind.TYPES);
    ...
    private static String readExpectedString(String resourcePath) throws IOException {
        try (InputStream in = ErlangUnionHelperIrTest.class.getClassLoader().getResourceAsStream(resourcePath)) {

...
class ErlangCredentialProviderIrTest {
    ...
    void resolveChainMatchesGolden() throws IOException {
        String combined = ErlangCredentialProviderIr.credentialFunctions().stream()
                .filter(fn -> fn.name().startsWith("resolve_chain")
                        || fn.name().equals("resolve_provider"))
    ...
    void credentialsModuleMatchesGolden() throws IOException {
        ...
        ErlModule module = ErlangCredentialProviderIr.credentialsModule("sigv4test_service_credentials", service);
    ...
    private static String readExpectedString(String resourcePath) throws IOException {
        try (InputStream in = ErlangCredentialProviderIrTest.class.getClassLoader().getResourceAsStream(resourcePath)) {

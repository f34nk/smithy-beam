...
/**
 * Extracts the @documentation trait value for a shape, cleaned for embedding in generated source
 * comments.
 */
public final class BeamDocumentation {
  ...
  /**
   * Returns the documentation string for the shape if the trait is present, with line breaks
   * preserved.
   */
  ...
  /** Writes shape-level documentation when the trait is present. No-op when absent. */
  public static void writeShapeDocIfPresent(
      SymbolWriter<?, ?> writer, Shape shape, DocTarget target) {
    forShape(shape)
        .ifPresent(
            doc -> {
              switch (target) {
  ...
  /**
   * Builds the Elixir {@code ## Members} appendix for documented structure members. Returns empty
   * when no member carries {@code @documentation}.
   */
  ...
  private static void writeElixirAttribute(
      SymbolWriter<?, ?> writer, String attribute, String doc) {
    if (!doc.contains("\n")) {

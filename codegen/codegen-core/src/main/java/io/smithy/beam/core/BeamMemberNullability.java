 ...
 *
 * <p>Dedicated operation input shapes carry {@code @input} and use {@link
 * NullableIndex.CheckMode#CLIENT}. Smithy honors {@code @clientOptional} in CLIENT mode and
 * {@code @default} according to the active check mode. No custom trait handling is required beyond
 * selecting the correct check mode.
 */
public final class BeamMemberNullability {
   ...
   *
   * <p>Input shapes use CLIENT mode so {@code @clientOptional} and {@code @default} flow through
   * {@link NullableIndex} as defined by the Smithy specification.
   */
  public static boolean isMemberNullable(
    ...
    NullableIndex.CheckMode mode =
        container.hasTrait(InputTrait.class)

 ...
 * Smithy 1.54.0 does not expose a public API for that registration, so this class reads the private
 * {@code transforms} and {@code model} fields via reflection. Field names and layout can change
 * across Smithy releases; {@code BeamCodegenTransformsTest} pins the expected names for the pinned
 * Smithy version.
 *
 * <p>When Smithy exposes public transform registration (for example {@code
 * director.registerTransform(...)}), replace the reflection helpers below rather than extending
 * them.
 */
public final class BeamCodegenTransforms {
  ...
  static {
    try {
      TRANSFORMS_FIELD = CodegenDirector.class.getDeclaredField(CODEGEN_DIRECTOR_TRANSFORMS_FIELD);
  ...
  /**
   * Registers transforms on the given director in a fixed order: default service codegen
   * simplification (mixin flatten, service errors copied to operations), dedicated operation input
   * and output shapes, optional relative deprecation filters from settings, removal of shapes
   * outside the service closure.
   *
   * <p>Call after {@code runner.model(...)}, {@code runner.service(...)}, and {@code
   * runner.settings(...)} (or {@code runner.settings(Class, Node)}) have been set.
   */
  public static <
      ...
      void applySharedCodegenTransforms(
          CodegenDirector<W, I, C, BeamSettings> director, BeamSettings settings) {
    ...
    ServiceShape service = model.expectShape(settings.resolveService(model), ServiceShape.class);
    pruneToServiceClosure(director, service);
  }

  /**
   * Removes shapes that are not in the closure of the given service from the director model during
   * {@link CodegenDirector#run()}.
   *
   * <p>Uses reflection to append to {@link CodegenDirector}'s private transform list because no
   * public registration hook exists on Smithy 1.54.0.
   */
  public static void pruneToServiceClosure(
    ...
    addDirectorTransform(
        director,
        (model, transformer) ->
            pruneModelToServiceClosure(model, model.expectShape(serviceId, ServiceShape.class)));
  }

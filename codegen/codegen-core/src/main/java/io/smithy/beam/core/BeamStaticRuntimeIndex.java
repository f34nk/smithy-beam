...
/**
 * Selects runtime modules required for a generated Erlang service.
 *
 ...
 */
public final class BeamStaticRuntimeIndex {

  public record Requirements(List<BeamStaticRuntimeModule> modules) {}

  private BeamStaticRuntimeIndex() {}
  ...
  private static Requirements requirements(
    ...
    if (client && BeamSigV4Metadata.from(service).isPresent()) {
      addWithDependencies(selected, BeamStaticRuntimeModule.AWS_SIGV4);
    }

    if (serviceHasHttpChecksum(model, service)) {
      addWithDependencies(selected, BeamStaticRuntimeModule.HTTP_CHECKSUM);
    }

    if (BeamEventStreamIndex.of(model).serviceHasEventStreams(service)) {
      addWithDependencies(selected, BeamStaticRuntimeModule.AWS_EVENT_STREAM);
    }
  ...
  private static void addWithDependencies(
      Set<BeamStaticRuntimeModule> selected, BeamStaticRuntimeModule module) {

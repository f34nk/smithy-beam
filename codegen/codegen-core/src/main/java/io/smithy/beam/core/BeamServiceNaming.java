...
public final class BeamServiceNaming {
  ...
  /**
   * Same as {@link #effectiveServiceSnakeName(BeamSettings, ServiceShape)} when the effective
   * service name string is already known (layout string constructor path).
   */
  public static String effectiveServiceSnakeName(
      BeamSettings settings, String effectiveServiceName) {
    return resolveServiceSnakeStem(settings, effectiveServiceName);
  }

  private static String resolveServiceSnakeStem(
      BeamSettings settings, String effectiveServiceName) {

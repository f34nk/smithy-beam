...
public final class BeamComplianceLiterals {
    ...
    public static String elixirStructLiteral(
        ...
        for (MemberShape member : shape.members()) {
            ...
            Node value = params.expectMember(memberName);
            String fieldName = BeamNameUtils.toSnakeCase(memberName);
            fields.add(fieldName + ": " + elixirValue(model, member, value, symbolProvider, structNameFn));
        }
    ...
    private static String enumAtom(Shape enumShape, String memberName, SymbolProvider symbolProvider) {
        ...
        Map<String, String> byMember = symbol.getProperty("enumAtomByMember", Map.class).orElse(Map.of());
    ...
    private static String intEnumMemberName(IntEnumShape shape, NumberNode numberNode) {
        ...
        for (MemberShape member : shape.members()) {
            int memberValue = member.getTrait(EnumValueTrait.class)
    ...
    private static String erlangMap(ObjectNode objectNode) {
        ...
        List<String> entries = new ArrayList<>();
        objectNode.getMembers().forEach((key, node) -> {

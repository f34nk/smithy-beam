...
public final class BeamEndpointRuleSetEmitter {
    ...
    public static Optional<String> serializeRuleSetErlangMap(Model model, ServiceShape service) {
        return resolveRuleSetTrait(model, service)
                .map(RuleSetSerializer::toErlangMap);
    }

    public static Optional<String> serializeRuleSetElixirMap(Model model, ServiceShape service) {
        return resolveRuleSetTrait(model, service)
                .map(RuleSetSerializer::toElixirMap);
    }

    public static Optional<String> serializeRuleSetJson(Model model, ServiceShape service) {
        return resolveRuleSetTrait(model, service)
                .map(trait -> RuleSetSerializer.toJson(model, trait));
    }

    public static Optional<EndpointRuleSetTrait> resolveRuleSetTrait(Model model, ServiceShape service) {
        return service.getTrait(EndpointRuleSetTrait.class)
                .or(() -> indirectRuleSetTrait(model, service));
    }

    private static Optional<EndpointRuleSetTrait> indirectRuleSetTrait(Model model, ServiceShape service) {

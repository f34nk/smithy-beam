...
class BeamHttpComplianceTestsTest {
    ...
    private static Model loadModel() {
        URL resource = BeamHttpComplianceTestsTest.class.getResource("/model/compliance_tests_fixture.smithy");
    ...
    void filterRequestTestsMatchesProtocolAndAppliesTo() {
        ...
        var filtered = BeamHttpComplianceTests.filterRequestTests(
                BeamHttpComplianceTests.requestTests(model, operation), AppliesTo.CLIENT, RestJson1Trait.ID);
        assertThat(filtered).hasSize(1);

        var serverFiltered = BeamHttpComplianceTests.filterRequestTests(
                BeamHttpComplianceTests.requestTests(model, operation), AppliesTo.SERVER, RestJson1Trait.ID);

...
class BeamHttpChecksumIndexTest {
  ...
  private static Model loadModel() {
    URL resource =
        BeamHttpChecksumIndexTest.class.getResource("/model/http_checksum_index_fixture.smithy");
  ...
  void requestChecksumsMapsRequiredTraitToContentMd5() {
    ...
    assertThat(index.requestChecksums(operation))
        .containsExactly(
  ...
  void responseChecksumsUsesModeledAlgorithms() {
    ...
    assertThat(index.responseChecksums(operation))
        .containsExactly(
            new BeamHttpChecksumIndex.ChecksumBinding(

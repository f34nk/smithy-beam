...
class BeamRetryIndexTest {
  ...
  void forErrorReturnsEmptyWhenNotAnErrorShape() {
    ...
    StructureShape unit =
        model.expectShape(ShapeId.from("smithy.beam.test.retry#Unit"), StructureShape.class);
  ...
  void forErrorClassifiesRetryableAndThrottlingTraits() {
    Model model = loadModel();
    StructureShape retryable =
        model.expectShape(
            ShapeId.from("smithy.beam.test.retry#RetryableError"), StructureShape.class);
    StructureShape throttling =
        model.expectShape(
            ShapeId.from("smithy.beam.test.retry#ThrottlingError"), StructureShape.class);
    StructureShape plain =
        model.expectShape(ShapeId.from("smithy.beam.test.retry#PlainError"), StructureShape.class);
    ...
    assertThat(BeamRetryIndex.forError(plain)).contains(new BeamRetryIndex.RetryInfo(false, false));
  }
}

...
class BeamMemberNullabilityTest {
  ...
  void dedicatedInputUsesClientMode() {
    ...
    StructureShape input =
        model.expectShape(ShapeId.from("smithy.beam.test#GetItemInput"), StructureShape.class);
  ...
  void clientModeHonorsClientOptionalAndDefault() {
    Model model =
        Model.assembler()
            ...
            .unwrap();
    StructureShape input =
        model.expectShape(ShapeId.from("smithy.beam.test#MixedInput"), StructureShape.class);
    NullableIndex index = NullableIndex.of(model);
    ...
    assertThat(BeamMemberNullability.isMemberNullable(index, input, requiredField)).isTrue();
    assertThat(BeamMemberNullability.isMemberNullable(index, input, optionalOnClient)).isTrue();

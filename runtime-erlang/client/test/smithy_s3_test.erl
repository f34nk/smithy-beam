...
is_localhost_test() ->
    ?assert(smithy_s3:is_localhost(<<"http://localhost:5050">>)),
    ?assert(smithy_s3:is_localhost(<<"http://localhost">>)),
    ?assert(smithy_s3:is_localhost(<<"https://localhost:4566">>)).

is_localhost_ip_test() ->
    ?assert(smithy_s3:is_localhost(<<"http://127.0.0.1:5050">>)),
    ?assert(smithy_s3:is_localhost(<<"http://127.0.0.1">>)),
    ?assert(smithy_s3:is_localhost(<<"https://192.168.1.1:9000">>)).

is_not_localhost_test() ->
    ?assertNot(smithy_s3:is_localhost(<<"https://s3.amazonaws.com">>)),
    ?assertNot(smithy_s3:is_localhost(<<"https://s3.us-east-1.amazonaws.com">>)),
    ?assertNot(smithy_s3:is_localhost(<<"https://bucket.s3.amazonaws.com">>)).

is_docker_host_test() ->
    ?assert(smithy_s3:is_localhost(<<"http://moto:5050">>)),
    ?assert(smithy_s3:is_localhost(<<"http://localstack:4566">>)),
    ?assert(smithy_s3:is_localhost(<<"http://minio:9000">>)).

use_path_style_localhost_test() ->
    ?assert(smithy_s3:use_path_style(<<"http://localhost:5050">>)),
    ?assert(smithy_s3:use_path_style(<<"http://127.0.0.1:9000">>)),
    ?assert(smithy_s3:use_path_style(<<"http://moto:5050">>)).

use_path_style_port_test() ->
    %% Any endpoint with a port should use path style
    ?assert(smithy_s3:use_path_style(<<"https://s3.amazonaws.com:443">>)),
    ?assert(smithy_s3:use_path_style(<<"http://custom-s3:9000">>)).

use_virtual_hosted_aws_test() ->
    %% Standard AWS endpoints should use virtual-hosted style
    ?assertNot(smithy_s3:use_path_style(<<"https://s3.amazonaws.com">>)),
    ?assertNot(smithy_s3:use_path_style(<<"https://s3.us-east-1.amazonaws.com">>)),
    ?assertNot(smithy_s3:use_path_style(<<"https://s3.eu-west-1.amazonaws.com">>)).
...
build_path_style_forced_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"my-bucket">>, <<"my-key">>, <<"">>, #{path_style => true}),
...
build_virtual_hosted_with_query_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"my-bucket">>, <<"my-key">>, <<"?versionId=456">>),
...
build_virtual_hosted_disabled_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"my-bucket">>, <<"my-key">>, <<"">>, #{path_style => false}),
...
special_characters_in_key_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"my-bucket">>, <<"path/to/file with spaces.txt">>, <<"">>),
...
empty_bucket_with_key_test() ->
    ...
    Client = #{endpoint => <<"http://localhost:5050">>},
    Url = smithy_s3:build_url(Client, <<>>, <<"some-key">>, <<"">>),
...
moto_put_object_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"test-bucket">>, <<"documents/report.pdf">>, <<"">>),
...
localstack_get_object_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"data-bucket">>, <<"data.json">>, <<"?versionId=abc123">>),
...
aws_production_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"prod-bucket">>, <<"logs/2024/01/app.log">>, <<"">>),
...
minio_test() ->
    ...
    Url = smithy_s3:build_url(Client, <<"local-bucket">>, <<"file.txt">>, <<"">>),

defmodule AwsSigv4 do
  ...
  @spec sign(HttpRequest.t(), map(), String.t(), String.t(), map()) :: HttpRequest.t()
  def sign(%HttpRequest{} = request, credentials, region, service, opts) do
    ...
    %{request | headers: from_erl_headers(signed_headers)}
  end

  @spec presign(HttpRequest.t(), map(), String.t(), String.t(), map()) ::
          {:ok, String.t()} | {:error, term()}
  def presign(%HttpRequest{} = request, credentials, region, service, opts) do
    access_key_id = Map.fetch!(credentials, :access_key_id)
    secret_access_key = Map.fetch!(credentials, :secret_access_key)
    datetime = :calendar.universal_time()
    host = resolve_host(request, opts)
  ...
  defp resolve_host(%HttpRequest{} = request, opts) do
    coalesce([
      request.host,
      Map.get(opts, :host),
  ...
  defp build_url(host, path, query) when map_size(query) == 0 do
    "https://#{host}#{path}"
  end
  ...
  defp to_bin(value), do: to_string(value)

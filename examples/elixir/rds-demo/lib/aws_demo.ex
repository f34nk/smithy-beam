defmodule AwsDemo do
  ...
  def run do
    ...
    param_group_created =
      case AwsRdsClient.create_db_parameter_group(client, create_input, %{enable_retry: false}) do
        {:ok, output} ->
          ...
          IO.puts("SUCCESS: Created parameter group: #{Map.get(created, "DBParameterGroupName", "unknown")}")
          true
        {:error, err} ->
          IO.puts("ERROR (expected in LocalStack free tier): #{inspect(err)}")
          false
    ...
    # 8. DescribeReservedDBInstancesOfferings
  ...
  # Handles three shapes returned by the query protocol:
  #   %{"OuterKey" => %{"InnerKey" => [...]}}  — wrapped list
  #   %{"OuterKey" => %{"InnerKey" => %{...}}} — single-item wrapped as map
  #   %{"OuterKey" => [...]}                   — direct list

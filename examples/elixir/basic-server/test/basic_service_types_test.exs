defmodule BasicTypesTest do
  ...
  describe "BasicNotFound error" do
    test "can be created with message and fault kind" do
      err = %BasicServiceTypes.BasicNotFound{message: "missing", __beam_error_kind: :client}
      assert err.message == "missing"
      assert err.__beam_error_kind == :client
    end

    test "defaults fault kind to client" do
      err = %BasicServiceTypes.BasicNotFound{message: "gone"}
      assert err.__beam_error_kind == :client
    end
  end
  ...
  describe "BasicStatus enum" do
    test "known wire values map to atoms" do
      assert BasicServiceTypes.BasicStatus.from_string("ACTIVE") == :active
      assert BasicServiceTypes.BasicStatus.from_string("INACTIVE") == :inactive
      assert BasicServiceTypes.BasicStatus.from_string("PENDING") == :pending
    end

    test "unknown wire values round-trip via {:unknown, value}" do
      assert BasicServiceTypes.BasicStatus.from_string("FUTURE_STATUS") ==
               {:unknown, "FUTURE_STATUS"}

      assert BasicServiceTypes.BasicStatus.to_string({:unknown, "FUTURE_STATUS"}) ==
               "FUTURE_STATUS"
    end

    test "values/0 returns all known atoms" do
      assert BasicServiceTypes.BasicStatus.values() == [:active, :inactive, :pending]
    end
  end
  ...
  describe "BasicPriority intEnum" do
    test "known integer values map to atoms" do
      assert BasicServiceTypes.BasicPriority.from_integer(1) == :low
      assert BasicServiceTypes.BasicPriority.from_integer(2) == :medium
      assert BasicServiceTypes.BasicPriority.from_integer(3) == :high
    end

    test "unknown integer values round-trip via {:unknown, value}" do
      assert BasicServiceTypes.BasicPriority.from_integer(99) == {:unknown, 99}
      assert BasicServiceTypes.BasicPriority.to_integer({:unknown, 99}) == 99
    end

    test "values/0 returns all known atoms" do
      assert BasicServiceTypes.BasicPriority.values() == [:low, :medium, :high]
    end
  end

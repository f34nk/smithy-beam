    ...
    codegenContext
        ...
        .useFileWriter(
            ...
            writer -> {
              ...
              writer.indent();
              writer.write("@moduledoc \"NDJSON protocol codec for $L.\"", service.getId());
              writer.write("");
              for (OperationShape op : operations) {
                Symbol opSym = sp.toSymbol(op);
                writer.write("def encode_$L_request(input) do", opSym.getName());
                writer.indent();
                writer.write("%{operation: :$L, input: input}", opSym.getName());
                writer.dedent();
                writer.write("end");
                writer.write("");
                writer.write("def decode_$L_response(_resp) do", opSym.getName());
                writer.indent();
                writer.write("{:ok, nil}");
                writer.dedent();
                writer.write("end");

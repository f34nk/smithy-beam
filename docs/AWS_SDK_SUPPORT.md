...
| [HTTP Protocol Compliance Tests](https://smithy.io/2.0/additional-specs/http-protocol-compliance-tests.html) | ✅ | Emits codec-level test modules from `@httpRequestTests` and `@httpResponseTests`, including forbid/require header and query checks, host expectations, and JSON body document compare. Does not cover `authScheme`, `vendorParams`, or transport-level Host resolution beyond the codec. |

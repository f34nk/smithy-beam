...
External dependency **`io.beam:beam-dsl`** provides `io.beam.dsl.erlang` and `io.beam.dsl.elixir` IR types used by the Erlang and Elixir codegen modules.
...
1. **Codegen-time** (Java on the JVM)
   ...
   - Language emitters compose structural IR trees from `beam-dsl` and render them to `.erl`/`.hrl` or `.ex` source text.

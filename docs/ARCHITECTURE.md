...
External dependency **`io.beam:beam-lang`** provides `io.beam.lang.erlang` and `io.beam.lang.elixir` IR types used by the Erlang and Elixir codegen modules.
...
1. **Codegen-time** (Java on the JVM)
   ...
   - Language emitters compose structural IR trees from `beam-lang` and render them to `.erl`/`.hrl` or `.ex` source text.
